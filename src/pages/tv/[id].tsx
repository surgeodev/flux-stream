...
import { getTVShow, imgPath, bgPath, getCatalog, getTVCredits, type MediaItem, type CastMember } from '@/hooks/use-tmdb'
...
export default function TVPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setShow(s)
      setPopular(cat.filter(c => c.type === 'tv').slice(0, 6))
      setCast(credits)
      setLoading(false)
    })
  }, [id])
  ...
  return (
    <Layout>
      ...
      <div className="container mx-auto px-4 md:px-6 -mt-32 md:-mt-40 relative z-10">
        <div className="flex flex-col md:flex-row gap-4 md:gap-10">
          <div className="w-28 md:w-64 flex-shrink-0 -mt-16 md:mt-0 self-start">
            <img src={poster} alt={show.title} className="w-full rounded-xl shadow-2xl border border-white/10" />
          </div>

          <div className="flex-1 pt-1 md:pt-20">
            <h1 className="text-2xl md:text-5xl font-extrabold font-display text-white drop-shadow-lg mb-3">{show.title}</h1>
            ...
            <div className="flex items-center gap-3 text-sm flex-wrap mb-4">
              {show.rating ? (
                <span className="flex items-center gap-1 text-yellow-400 bg-black/50 px-2.5 py-1 rounded-md backdrop-blur border border-white/10">
                  <Star className="w-3.5 h-3.5 fill-current" />{show.rating.toFixed(1)}
                </span>
              ) : null}
              <span className="bg-white/10 px-2.5 py-1 rounded-md backdrop-blur border border-white/10 text-xs">{show.year || ''}</span>
              {show.genres?.map(g => (
              ...
              {show.seasons ? <span className="bg-white/10 px-2.5 py-1 rounded-md backdrop-blur border border-white/10 text-xs">{show.seasons} saisons</span> : null}
            </div>

            {show.overview && <p className="text-sm text-white/70 max-w-2xl leading-relaxed mb-6">{show.overview}</p>}
      ...
      <section className="container mx-auto px-4 md:px-6 py-10">
        <h2 className="text-xl font-bold font-display mb-6">Épisodes</h2>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <select value={selectedSeason} onChange={e => setSelectedSeason(Number(e.target.value))}
            className="bg-card border border-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 md:gap-3">
          {episodes.map(ep => (
            <Link
              key={ep}
              href={`/watch?type=tv&id=${show.id}&title=${encodeURIComponent(show.title)}&s=${selectedSeason}&e=${ep}`}
              className={`flex flex-col p-3 border rounded-lg transition-all hover:border-primary/50 hover:bg-white/5 ${ep === selectedEpisode ? 'border-primary bg-primary/10' : 'border-border bg-card'}`}
              onClick={() => setSelectedEpisode(ep)}
            >
              <span className="text-xs font-bold text-primary mb-1">Ép. {ep}</span>
              <span className="text-sm font-medium">S{selectedSeason}E{ep}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {popular.length > 0 && (
        <section className="container mx-auto px-4 md:px-6 py-10">
          <h2 className="text-xl font-bold font-display mb-6">Séries populaires</h2>
